...
from synthetic_data import (
    ...
    with_selected_metrics,
)


st.set_page_config(page_title="Fairness Measures Explorer", layout="wide")
...
def metric_selector(label: str, category: str, default_keys: list[str] | None = None):
    ...
    default = default_keys or options
...
def render_synthetic_page() -> None:
    ...
    with st.sidebar:
        ...
        total = st.number_input("n (total samples)", min_value=1, value=24, step=1, key="synthetic_total")
        estimated_rows = count_confusion_matrices(int(total))
        st.caption(f"All possible confusion matrices: {estimated_rows:,}")
        if estimated_rows > 3_000_000:
        ...
        uploaded_pickle = st.file_uploader("Optional pickle from Set(08,n).bin or this app", type=["bin", "pkl"], key="synthetic_pickle")

        if st.button("Build synthetic dataset", type="primary"):
            try:
                if synth_mode == "Exact enumeration":
                    df = generate_exact_confusion_matrices(int(total), max_rows=int(max_exact_rows))
                elif synth_mode == "Monte Carlo sample":
                    df = sample_uniform_confusion_matrices(int(total), int(monte_carlo_draws), seed=int(seed))
                else:
                    if uploaded_pickle is None:
                        raise ValueError("Upload a pickle file first.")
                    df = load_confusion_matrices_from_pickle(uploaded_pickle.getvalue())
                    total = int(df.iloc[0].sum())
                st.session_state["synthetic_df"] = add_base_columns(df)
                st.session_state["synthetic_total"] = int(total)
    ...
    with tabs[0]:
        ...
        available_gr = ratio_values(synthetic_total)
        available_ir = ratio_values(synthetic_total)
        ...
        selected_ir = st.multiselect(
            "IR panel values",
            options=available_ir,
            default=list(reversed(default_grid)),
            format_func=ratio_label,
...
def render_case_study_page() -> None:
    ...
    with st.sidebar:
        ...
        adult_source_mode = st.radio("Adult dataset source", options=["Upload file", "Local path"], key="adult_source_mode")
        uploaded = None
        ...
        if st.button("Run Adult case study", type="primary"):
            try:
                ...
                fairness_results, performance_results = evaluate_case_study(
                    ...
                    fairness_metric_keys=selected_fairness_metrics,
    ...
    with tabs[0]:
        ...
        fig1 = plot_case_line(
            fairness_results,
            metric_key,
            fairness_label_map.get(metric_key, metric_key),
            ratio_type,
            fill=fill,
        )
        ...
        if subset.empty:
            ...
            for clf_name, group in subset.groupby("clf"):
                fairness_map[clf_name] = {
                    fairness_label_map.get(metric_key, metric_key): float(group[group["metric"] == metric_key]["value"].mean())
                    for metric_key in pd.unique(subset["metric"])
                }
